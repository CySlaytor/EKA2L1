...
namespace eka2l1 {
    ...
    void window_server::parse_wsini() {
        ...
        do {
            ...
            do {
                ...
                if (mode_width) {
                    mode_width->get_as<common::ini_pair>()->get(reinterpret_cast<std::uint32_t *>(&scr_mode.size.x), 1, 0);
                } else {
                    if (total_mode > 1)
                        break;
                ...
                if (mode_height) {
                    mode_height->get_as<common::ini_pair>()->get(reinterpret_cast<std::uint32_t *>(&scr_mode.size.y), 1, 0);
                } else {
                    if (total_mode > 1)
                        break;
                ...
                if (mode_rot) {
                    mode_rot->get_as<common::ini_pair>()->get(reinterpret_cast<std::uint32_t *>(&scr_mode.rotation), 1, 0);
                } else {
                    scr_mode.rotation = 0;
                }
            ...
            do {
                ...
                common::ini_node_ptr mode_normal_node = screen_node->get_as<common::ini_section>()->find(screen_mode_normal_mode_num_key.c_str());
                ...
                common::ini_node_ptr mode_alternate_node = screen_node->get_as<common::ini_section>()->find(screen_mode_alternate_mode_num_key.c_str());
    ...
    bool make_key_event(epoc::key_map &map, drivers::input_event &driver_evt_, epoc::event &guest_evt_) {
        ...
        guest_evt_.type = (driver_evt_.key_.state_ == drivers::key_state::released) ? epoc::event_code::key_up : epoc::event_code::key_down;
