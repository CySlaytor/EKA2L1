...
namespace eka2l1 {
    ...
    void mmf_dev_server_session::max_volume(service::ipc_context *ctx) {
        std::optional<epoc::mmf_dev_sound_proxy_settings> settings = ctx->get_argument_data_from_descriptor<epoc::mmf_dev_sound_proxy_settings>(2);
        if (!settings) {
    ...
    void mmf_dev_server_session::set_volume(service::ipc_context *ctx) {
        std::optional<epoc::mmf_dev_sound_proxy_settings> settings = ctx->get_argument_data_from_descriptor<epoc::mmf_dev_sound_proxy_settings>(1);
        if (!settings) {
            ctx->complete(epoc::error_argument);
            return;
        }
    ...
    void mmf_dev_server_session::init3(service::ipc_context *ctx) {
        ...
        std::optional<epoc::mmf_dev_sound_proxy_settings> state_settings = ctx->get_argument_data_from_descriptor<epoc::mmf_dev_sound_proxy_settings>(1);
        ...
        if ((state_wanted != epoc::mmf_state_playing) && (state_wanted != epoc::mmf_state_tone_playing) && (state_wanted != epoc::mmf_state_playing_recording)) {
        ...
        if (!stream_->format(target_fourcc)) {
            ...
            if (!evt_msg_queue_)
                ...
                send_event_to_msg_queue(epoc::mmf_dev_sound_queue_item(epoc::mmf_dev_newarch_init_complete_evt, epoc::error_general));
                ctx->complete(epoc::error_none);
            }
            return;
        }
    ...
    void mmf_dev_server_session::set_config(service::ipc_context *ctx) {
        ...
        conf_.buffer_size_ = static_cast<std::int32_t>(common::clamp(MMF_BUFFER_SIZE_MIN, MMF_BUFFER_SIZE_MAX, recommended_buffer_size(freq)));
    ...
    void mmf_dev_server_session::copy_fourcc_array(service::ipc_context *ctx) {
        ctx->write_data_to_descriptor_argument(2, reinterpret_cast<const std::uint8_t *>(four_ccs_.data()), static_cast<std::uint32_t>(four_ccs_.size() * sizeof(std::uint32_t)));
        ctx->complete(epoc::error_none);
    }
    ...
    void mmf_dev_server_session::play_data(service::ipc_context *ctx) {
        ...
        if (((supplied_size * 16) == conf_.buffer_size_) && (out_stream->format() == drivers::PCM16_FOUR_CC_CODE)) {
            conf_.buffer_size_ = common::clamp<std::int32_t>(MMF_BUFFER_SIZE_MIN, MMF_BUFFER_SIZE_MAX, conf_.buffer_size_ * 16);
        }
        ctx->complete(epoc::error_none);
    }
