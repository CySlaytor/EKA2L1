...
namespace eka2l1::epoc {
  ...
  static void initialize_system_properties(eka2l1::system *sys, eka2l1::config::state *cfg) {
    ...
    if (dvcs.size() > cfg->device) {
      ...
      if (cfg->language == -1) lang.language = static_cast<epoc::language>(dvc.default_language_code);
      else lang.language = static_cast<epoc::language>(cfg->language);
    }
...
namespace eka2l1 {
  namespace service {
    void init_services(system *sys) {
      ...
      if (cfg->enable_srv_rights) CREATE_SERVER(sys, rights_server);
      if (cfg->enable_srv_sa) CREATE_SERVER(sys, sa_server);
      if (cfg->enable_srv_drm) CREATE_SERVER(sys, drm_helper_server);
      ...
      // Force instantiation of EKA2 (OS 9.3) specific servers
