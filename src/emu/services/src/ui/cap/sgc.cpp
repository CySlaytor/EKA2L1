...
namespace eka2l1::epoc::cap {
    ...
    bool sgc_server::wg_state::is_fullscreen() const { return flags_.get(FLAG_FULLSCREEN); }
    ...
    bool sgc_server::wg_state::is_legacy_layout() const { return flags_.get(FLAG_LEGACY_LAYOUT); }
    ...
    bool sgc_server::wg_state::understands_partial_foreground() const { return flags_.get(FLAG_UNDERSTAND_PARTIAL_FOREGROUND); }
    ...
    bool sgc_server::wg_state::orientation_specified() const { return flags_.get(FLAG_ORIENTATION_SPECIFIED); }
    ...
    bool sgc_server::wg_state::orientation_landscape() const { return flags_.get(FLAG_ORIENTATION_LANDSCAPE); }
    ...
    void sgc_server::change_wg_param(const std::uint32_t id, wg_state::wg_state_flags &flags, const std::int32_t sp_layout,
        ...
        state->set_fullscreen(flags.get(0));
        state->set_legacy_layout(flags.get(1));
        state->sp_layout_ = sp_layout;
        state->sp_flags_ = sp_flags;
        state->set_understand_partial_foreground(true);
        state->set_orientation_specified(flags.get(2));
        state->set_orientation_landspace(flags.get(3));
