...
namespace eka2l1::epoc {
    ...
    void skn_file::process_image_table_def_chunk(std::uint32_t base_offset) {
        ...
        std::uint32_t offset_of_attrib = base_offset + skn_desc_dfo_img_table_image_major0 + count * skn_desc_dfo_img_table_image_size;
        ...
        for (std::uint16_t i = 0; i < count; i++) {
            ...
            stream_->read(base_offset + skn_desc_dfo_img_table_image_major0 + i * skn_desc_dfo_img_table_image_size, &img_hash, 8);
    ...
    void skn_file::process_color_table_def_chunk(std::uint32_t base_offset) {
        ...
        std::uint32_t offset_of_attrib = base_offset + skn_desc_dfo_color_tab_color_idx0 + count * skn_desc_dfo_color_tab_color_size;
        process_attrib(offset_of_attrib, tab_.attrib);

        skn_color_table &tab_ref_ = color_tabs_.emplace(tab_.id_hash, std::move(tab_)).first->second;
        for (std::uint16_t i = 0; i < count; i++) {
            std::int16_t idx = 0;
            stream_->read(base_offset + skn_desc_dfo_color_tab_color_idx0 + i * skn_desc_dfo_color_tab_color_size, &idx, 2);
            common::rgba color = 0;
            stream_->read(base_offset + skn_desc_dfo_color_tab_color_rgb0 + i * skn_desc_dfo_color_tab_color_size, &color, 4);
    ...
    void skn_file::process_bitmap_anim_def_chunk(std::uint32_t base_offset) {
        ...
        std::uint32_t offset_of_attrib = base_offset + skn_desc_dfo_bmp_anim_frame_major0 + count * skn_desc_dfo_bmp_anim_frame_size;
        ...
        for (std::int16_t i = 0; i < count; i++) {
            ...
            stream_->read(base_offset + skn_desc_dfo_bmp_anim_frame_major0 + i * skn_desc_dfo_bmp_anim_frame_size, &frame.frame_bmp_hash, 8);
            stream_->read(base_offset + skn_desc_dfo_bmp_anim_frame_time0 + i * skn_desc_dfo_bmp_anim_frame_size, &frame.time, 2);
            stream_->read(base_offset + skn_desc_dfo_bmp_anim_frame_posx0 + i * skn_desc_dfo_bmp_anim_frame_size, &frame.posx, 2);
            stream_->read(base_offset + skn_desc_dfo_bmp_anim_frame_posy0 + i * skn_desc_dfo_bmp_anim_frame_size, &frame.posy, 2);
    ...
    void skn_file::process_effect_queue_chunk(std::uint32_t base_offset) {
        ...
        stream_->read(base_offset + skn_desc_dfo_effect_queue_input_layer_mode, &effect_queue.input_layer_mode, 1);
