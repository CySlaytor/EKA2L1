...
namespace eka2l1::epoc {
    ...
    std::u16string pid_to_string(const epoc::pid skin_pid) {
        std::string result;
        result += fmt::format("{:08x}", static_cast<std::uint32_t>(skin_pid.first));
        if (skin_pid.second) {
            result += fmt::format("{:08x}", static_cast<std::uint32_t>(skin_pid.second));
        }
        return common::utf8_to_ucs2(result);
    }
